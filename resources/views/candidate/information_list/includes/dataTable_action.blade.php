<div class="row">
    <div class="hstack gap-2">
        ...
        <a href="{{ route($params['base_route'].'edit',$params['id']) }}" title="Edit"
           ...
           {{$params['authorized_agency_id'] != auth()->user()->authorized_agency_id ? 'disable-button':''}}"><i class="ri-pencil-fill"></i></a>
        <button class="btn btn-outline-danger waves-effect waves-light cs-remove-data" title="Remove"
                {{$params['authorized_agency_id'] != auth()->user()->authorized_agency_id ? 'disabled':''}}
